...
# # self.input 的数据复制到 self.input_pad 的这个中心子区域。
# # self.input_pad 的其余部分隐式地填充了它初始化时的默认值（可能是零）。
# #self.input: 这是一个四维张量，表示输入数据。可以假设其形状为 
# # (batch_size, channels, height, width)，
# # 即 (批次大小，通道数，高度，宽度)。对应.shape[0]、.shape[1]、.shape[2]、.shape[3]、
...
class MaxPoolingLayer(object):
    ...
    def forward(self, input):
        ...
        return self.output
# self.input[idxn, idxc, h_start:h_end, w_start:w_end]: 这表示从输入张量 self.input 中提取一个子区域。
# 具体来说，它选择了 self.input 中第 idxn 个批次、第 idxc 个通道、从 h_start 到 h_end-1 的高度范围
# 以及从 w_start 到 w_end-1 的宽度范围的子区域。
# self.output[idxn, idxc, idxh, idxw] = ...: 这是赋值操作，将计算出的最大值赋值给输出张量 self.output 的某个位置。
# 具体来说，它将最大值赋值给 self.output 中第 idxn 个批次、第 idxc 个通道、第 idxh 个高度位置和第 idxw 个宽度位置。
 

#Flatten层来将这些高维特征图转换成一维数组
class FlattenLayer(object):
    ...
    def forward(self, input):   # 前向传播的计算
        ...
        self.input = np.transpose(input,(0,2,3,1))   # 转换为matconvnet的维度格式
