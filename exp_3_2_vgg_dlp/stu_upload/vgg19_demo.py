...
class VGG19(object):
    ...
    def build_model(self, param_path='../../imagenet-vgg-verydeep-19.mat'):
        ...
        # conv1_2
        #input_shape12=pycnnl.IntVector(4): 创建一个长度为4的整数向量input_shape12，用于存储卷积层的输入数据的形状。
        # input_shape12[0]=1: 设置输入数据的批次大小（batch size）为1。
        # input_shape12[1]=64: 设置输入数据的通道数（channel）为64。
        # input_shape12[2]=224: 设置输入数据的高度为224像素。
        # input_shape12[3]=224: 设置输入数据的宽度为224像素。
        # 第三个参数64是输出通道数，即该卷积层将会输出64个特征图（feature maps）。
        # 第四个参数3是卷积核的大小，这里使用的是3x3的卷积核。
        # 第五个参数1是卷积核在输入数据上的滑动步长（stride），这里步长为1，意味着卷积核每次移动1个像素。
        # 第六个参数1是卷积操作在输入数据边缘的填充（padding）大小，这里使用的是1个像素的填充，以保持输出数据的空间维度与输入相同（或尽可能接近）。
        # 第七个参数1通常用于指定是否使用偏置项（bias），这里设置为1表示使用偏置项。
