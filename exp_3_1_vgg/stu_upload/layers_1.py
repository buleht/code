...
class FullyConnectedLayer(object):
    ...
    def backward(self, top_diff):   # 反向传播的计算
        ...
        #d_weight是y对于w的偏导数矩阵，top_diff是上一层导数矩阵？上一次的bottom_diff？
        self.d_weight=np.matmul(self.input.T,top_diff)#self.input.T是输入矩阵的转置，可以联想一维矩阵，此时转置实际上是yi对于w矩阵对应列的导数
        #d_bias是y对于b的偏导数矩阵
        # self.d_bias=np.matmul(top_diff,axis=0,keepdims=True)
