...
#这里创建了一个卷积层实例，命名为 'conv1_1'。
# 第一个 3 可能表示输入图像的通道数（假设是RGB图像，因此有3个通道）。
# 第二个 3 表示卷积核的大小（3x3）。
# 64 表示输出通道数，即该层将生成64个特征图。
# 最后的两个 1 分别表示卷积的步长（stride）和填充（padding）大小。
    ...
    def load_model(self):
        ...
        for idx in range(43):
            if 'conv' in self.param_layer_name[idx]:
                ...
                # TODO：调整参数的形状
                #  [out_channel, height, width, in_channel] -> [in_channel, height, width, out_channel]
                weight = np.transpose(weight,[2,0,1,3])
                bias = bias.reshape(-1)  #转换偏置形状
                self.layers[self.param_layer_name[idx]].load_param(weight, bias)
            if idx >= 37 and 'fc' in self.param_layer_name[idx]:
                # 对于全连接层，假设MatConvNet的输出是[out_features, in_features]
                # 我们需要转换成[in_features, out_features]
                ...
                #weight =np.transpose(weight)   # 调整参数的形状
                self.layers[self.param_layer_name[idx]].load_param(weight, bias)
    ...
    def load_image(self, image_dir):
        ...
        # TODO：调整图片维度顺序为[N, height, width, channel]
